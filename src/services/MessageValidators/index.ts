import { IFileData, ISearchMessage } from '../../types/meta';
...
export class MessageValidatorService {
    ...
    public static validateSearchMessage(searchMessage: ISearchMessage): void {
        if (!searchMessage.query || searchMessage.query.trim().length === 0) {
            throw new Error('Search message cannot be empty');
        }

        if (searchMessage.query.length > 100) {
